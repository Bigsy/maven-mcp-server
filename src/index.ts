...
class MavenDepsServer {
    ...
    private async fetchLatestFromMetadata(coord: MavenCoordinate, excludePreReleases: boolean): Promise<string | null> {
        try {
            // Construct the maven-metadata.xml URL
            const groupPath = coord.groupId.replace(/\./g, '/');
            const metadataUrl = `https://repo1.maven.org/maven2/${groupPath}/${coord.artifactId}/maven-metadata.xml`;
            ...
            if (!metadata.metadata?.versioning?.[0]) {
                return null;
            }

            const versioning = metadata.metadata.versioning[0];

            // If we need to exclude pre-releases, filter the versions list
            if (excludePreReleases && versioning.versions?.[0]?.version) {
                const allVersions = versioning.versions[0].version;
                const stableVersions = allVersions.filter(v => !isPreReleaseVersion(v));

                if (stableVersions.length > 0) {
                    // Return the last stable version in the list (most recent)
                    return stableVersions[stableVersions.length - 1];
                }
                return null;
            }

            // Return the latest version (which may include pre-releases)
            if (versioning.latest?.[0]) {
                return versioning.latest[0];
            }

            if (versioning.release?.[0]) {
                return versioning.release[0];
            }

            return null;
        } catch (error) {
            // If metadata fetch fails, return null to fall back to Solr API
            return null;
        }
    }
    ...
    private async handleGetLatestRelease(args: unknown) {
        ...
        try {
            // First, try to get the latest version from maven-metadata.xml
            const metadataVersion = await this.fetchLatestFromMetadata(coord, excludePreReleases);
            ...
            if (metadataVersion) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: metadataVersion,
                        },
                    ],
                };
            }

            // Fall back to Solr API if metadata fetch failed
    ...
    private async handleListVersions(args: unknown) {
        ...
        try {
            // Try to get versions from maven-metadata.xml first
            try {
                const groupPath = coord.groupId.replace(/\./g, '/');
                const metadataUrl = `https://repo1.maven.org/maven2/${groupPath}/${coord.artifactId}/maven-metadata.xml`;
                ...
                if (metadata.metadata?.versioning?.[0]?.versions?.[0]?.version) {
                    let allVersions = metadata.metadata.versioning[0].versions[0].version;

                    // Filter pre-releases if needed
                    if (excludePreReleases) {
                        allVersions = allVersions.filter(v => !isPreReleaseVersion(v));
                    }

                    if (allVersions.length > 0) {
                        // Reverse to get newest first, then slice to depth
                        const versions = allVersions.reverse().slice(0, depth);
                        return {
                            content: [
                                {
                                    type: 'text',
                ...
                // Fall through to Solr API
            }

            // Fall back to Solr API
            let query = `g:"${coord.groupId}" AND a:"${coord.artifactId}"`;
            if (coord.packaging) {
                query += ` AND p:"${coord.packaging}"`;
            }
